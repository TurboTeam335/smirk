...
searchButton.addEventListener("click", async function () {
  ...
  try {
    await display_results_temp(searchText);
  } catch (err) {
...
function store(newStock) {
  ...
  if (!historyArray.map(stock => stock.toUpperCase()).includes(newStock)) {
...
function removeLocalStorage(valueToRemove) {
  ...
  const filteredValue = history.filter(function (item) {
    return item.toUpperCase() !== valueToRemove.toUpperCase();
  });
...
async function display_results(ticker) {
  ...
  if (data["pointsChanged"] > 0) {
    arrow =
      '<i class="arrow fas fa-arrow-up text-success"></i>' +
      data["percChanged"];
  }
  if (data["pointsChanged"] < 0) {
    console.log("down");
...
async function display_results_temp(ticker) {
  try {
    const data = await returnStockData(ticker.toUpperCase());
    let arrow = '';
    ...
    document.getElementById("rando_stock").innerHTML = '<p>Invalid Ticker, please search again.</p>';
  }
}
